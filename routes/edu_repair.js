...
router.get('/pts_all_list', authenticate, (req, res) => {
    rp.get({ url: `${process.env.VACANCIES_HOST}:${process.env.VACANCIES_PORT}/api/student_restor/all_list`, form: {}, 
...
router.post('/get_user_restored_data', authenticate, (req, res) => {
    rp.get({ url: `${process.env.VACANCIES_HOST}:${process.env.VACANCIES_PORT}/api/student_restor/get_user_restored_data/${req.body.fin}`, 
...
router.post('/save', authenticate, (req, res) => {
    ...
    db.notifications.create({ service: 'edu_repair', fin: result.id, title: (Number(status) ? 1 : 0), 
    description: Number(status) ? "Sizin müraciət qeydə alındı. " 
    : "Siz müraciətinizi tamamlamamısınız. Müraciətin qeydə alınması üçün zəhmət olmasa müraciətinizi tamamlayasınız." }).then(() => {
    });
    });
