...
router.get('/appealed_vacancies', authenticate, (req, res) => {   
    db.appealed_vacancies.findAll({where:{user_id:req.currentUser.id}, order:[['priority', 'ASC']] , 
    include:[{model:db.vacancy_appeals, required:false, attributes:['is_director'], where:{year:req.query.year}}]}).then(result => {
    res.json(result);
    });
});
...
router.post('/save', authenticate, (req, res) => {
    ...
    description: 'Müraciətiniz qeydə alındı. Hazırda Sizin müraciətinizə baxılır.' }).then(() => { });
    }
    }
    resultCount++; if (resultCount == 9) { resolve(true); }
    });
