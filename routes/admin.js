...
router.get('/getUser/:fin', authenticate, (req, res) => {
  ...
  if (isAdmin) { 
  db.users.findOne({attributes:['id', 'email', 'role', 'phone', 'country_code', 
  'citizenshipId', 'asanLogin'], where:{fin}, 
  ...
  db.users.findOne({attributes:['id', 'email', 'role', 'phone', 'country_code', 
  'citizenshipId', 'asanLogin'], where:{id:children.user_id}, include:[
  {model:db.fin_data, required: false}]}).then(children_user => {
  res.json({ user: user ? toAuthJSON(user) : null, children, 
  children_user: children_user ? toAuthJSON(children_user) : null })});
  else
...
router.post('/email_update', authenticate, (req, res) => {
  ...
  //admin_update_log 
  insert('admin_update_log', { old: check.email, new: email, 
  fin, description, user_id: req.currentUser.id }, () => {
  res.json({ message: !isEng ? 'E-poçt uğurla dəyişdirildi!' 
